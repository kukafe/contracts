...
// File contracts/StrategyLPReferralHarvestLock.sol
...
interface IMasterChefwLockupReferrer {
    
    function deposit(uint256 poolId, uint256 amount, address referral) external;
 ...
 */
contract StrategyLPReferralHarvestLock is Ownable, Pausable {
    ...
    uint256 public MIN_TO_LIQUIFY = 0;
    ...
    function getRewardState() public view returns (uint256 amount, uint256 rewardLockedUp, uint256 nextHarvest) {
        (amount,,rewardLockedUp,nextHarvest) = IMasterChefwLockupReferrer(masterchef).userInfo(poolId, address(this));
    }
    function setSwapPathRegisty(address _a) external onlyOwner{
        swapPathRegistry = _a;
    }
    ...
    function deposit() public{
        ...
        if (pairBal > 0) {
            IMasterChefwLockupReferrer(masterchef).deposit(poolId, pairBal, address(0));
    ...
    function withdraw(uint256 _amount) external onlyUser{
        ...
        if (pairBal < _amount) {   
            IMasterChefwLockupReferrer(masterchef).withdraw(poolId, _amount.sub(pairBal));
    ...
    function exit() external onlyUser{
        IMasterChefwLockupReferrer(masterchef).withdraw(poolId, balanceOfPool());
        uint256 pairBal = IERC20(lpPair).balanceOf(address(this));
        IERC20(lpPair).safeTransfer(user, pairBal);
    ...
    function addliquidity() internal{
        ...
        KCSRouter2(router).swapExactTokensForTokensSupportingFeeOnTransferTokens(amtToSell, 0, path, address(this), now+600);
        ...
        if (otherToken != rewardToken){
            ...
            KCSRouter2(router).swapExactTokensForTokensSupportingFeeOnTransferTokens(amtToSell, 0, path2, address(this), now+600);
        }

        KCSRouter2(router).addLiquidity(otherToken, baseToken, IERC20(otherToken).balanceOf(address(this)), IERC20(baseToken).balanceOf(address(this)), 0, 0, address(this), now+600);
    }
    ...
    function canExitHarvest() external view returns (bool) {
        ...
        uint256 rewardBal = IERC20(rewardToken).balanceOf(address(this));
        (uint256 amount, uint256 rewardLockedUp, uint256 nextHarvest) = getRewardState();
        return 
            (IMasterChefwLockupReferrer(masterchef).canHarvest(poolId, address(this)) && rewardLockedUp > 0) ||
            (balanceOfPool() > 0) || 
            (rewardBal > 0);
    ...
    function _harvest() internal whenNotPaused {
        ...
        if (!exitMode && IMasterChefwLockupReferrer(masterchef).canHarvest(poolId, address(this))){
            IMasterChefwLockupReferrer(masterchef).deposit(poolId, 0, address(0));
    ...
    /// conveninece function to match signature of the normal vaults
    ...
    //     require(u==user, "function only for user");
    //     return balanceOf();
    // }
    ...
    function balanceOfPool() public view returns (uint256) {
        (uint256 _amount,,,) = IMasterChefwLockupReferrer(masterchef).userInfo(poolId, address(this));
        return _amount;
    }
...
// File contracts/PrivateVaultFactory_LPReferralHarvestLock.sol
...
contract PrivateVaultFactory_LPReferralHarvestLock is Ownable {
    ...
    string public constant VAULT_TYPE="LPReferralHarvestLock";
    constructor() public {
    }
    ...
    function createVault(address _lpPair, 
        ...
        address _baseToken, 
        address _masterchef, 
        uint256 _poolId, address _router, address _swapPathRegistry, address _user, address _feeStrat) external returns (address) {
            
        StrategyLPReferralHarvestLock temp = new StrategyLPReferralHarvestLock (_lpPair,
            _rewardToken,
            _baseToken,
            _masterchef,
            _poolId,
