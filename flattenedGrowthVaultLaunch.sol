...
interface IStrategy {
    ...
    function setRouteMappings(address _from, address _to, address[] memory path) external;
 ...
 */
contract GrowthVaultLaunch is ERC20, Ownable {
    ...
    function withdraw(uint256 _shares) public {
        ...
        if (buybackstrat != address(0)){
            uint256 fee = r.mul(Buybackstrat(buybackstrat).withdrawalFeesBps()).div(10000);
            r = r.sub(fee);
    ...
    function setRouteMappings(address _from, address _to, address[] memory path) external onlyOwner {
        IStrategy(strategy).setRouteMappings(_from,_to, path);
    }
