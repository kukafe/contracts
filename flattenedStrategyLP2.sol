...
contract StrategyLP2 is Ownable, Pausable {
    ...
    uint256 public MIN_TO_LIQUIFY = 0 ether;
    bool public stakingMode = false;

    mapping(address => mapping (address => address[])) public routeMappings;

    function setStakingMode(bool _b) external onlyOwner{
        stakingMode = _b;
    }

    function setRouteMappings(address _from, address _to, address[] memory path) external onlyOwner {
        routeMappings[_from][_to] = path;
    }
    ...
    address _vault, 
    address _masterchef, 
    uint256 _poolId, address _router) public {
        lpPair = _lpPair;
    ...
    function deposit() public {
        ...
        if (pairBal > 0) {
            (stakingMode)?
                IMasterChefWStaking(masterchef).enterStaking(pairBal) :
                IMasterChefWStaking(masterchef).deposit(poolId, pairBal);
        }
    }
    ...
    function withdraw(uint256 _amount) external {
        ...
        if (pairBal < _amount) {   
            (stakingMode)?
                IMasterChefWStaking(masterchef).leaveStaking(_amount.sub(pairBal)):
                IMasterChefWStaking(masterchef).withdraw(poolId, _amount.sub(pairBal));
    ...
    function addliquidity() internal{
        ...
        KCSRouter2(router).swapExactTokensForTokensSupportingFeeOnTransferTokens(amtToSell, 0, routeMappings[rewardToken][baseToken], address(this), now);

        if (otherToken != rewardToken){
            KCSRouter2(router).swapExactTokensForTokensSupportingFeeOnTransferTokens(amtToSell, 0, routeMappings[rewardToken][otherToken], address(this), now);
        }
    ...
    function swapToStakingToken() internal{
        if (rewardToken != lpPair){
            uint amount = IERC20(rewardToken).balanceOf(address(this));
            KCSRouter2(router).swapExactTokensForTokensSupportingFeeOnTransferTokens(amount, 0, routeMappings[rewardToken][lpPair], address(this), now);
        }
    }
    ...
    function _harvest() internal whenNotPaused {
        (stakingMode)?
            IMasterChefWStaking(masterchef).enterStaking(0):
            IMasterChefWStaking(masterchef).deposit(poolId, 0);
    ...
    function getKCSValue(address user, address[] memory path) public view returns (uint256){
        uint256[] memory amts = KCSRouter2(router).getAmountsOut(1 ether, path);
        uint256 temp = amts[amts.length - 1];
        return temp.mul(balanceOf()).div(1e18);
    }
    /**
    ...
    function balanceOfPool() public view returns (uint256) {
        (uint256 _amount,) = IMasterChefWStaking(masterchef).userInfo(poolId, address(this));
        return _amount;
    }
    ...
    function panic() public onlyOwner {
        pause();
        (stakingMode)?
            IMasterChefWStaking(masterchef).leaveStaking(balanceOfPool()):
            IMasterChefWStaking(masterchef).withdraw(poolId,balanceOfPool());
    }
